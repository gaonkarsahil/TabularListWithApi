...
function loadTable() {
  ...
  xhttp.open("GET", "http://localhost:8000/api/v1/pldata");
  xhttp.send();
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      for (let object of rows) {
        ...
        trHTML += '<td>'+object['sold_quantity']+'</td>';
        trHTML += '<td>'+object['holding_quantity']+'</td>';
        trHTML += '<td>'+object['total_buy_amount']+'</td>';
        trHTML += '<td>'+object['total_sell_amount']+'</td>';
        trHTML += '<td>'+object['holding_amount']+'</td>';
        trHTML += '<td>'+object['unit_hold_amount']+'</td>';

        trHTML += '<td>'+object['open']+'</td>';
        trHTML += '<td>'+object['close']+'</td>';
        trHTML += '<td>'+object['pl_amount']+'</td>';
        trHTML += '<td>'+object['free_stock']+'</td>';
